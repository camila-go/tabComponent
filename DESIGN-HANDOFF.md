...
4. **Tab icons** — ✅ implemented with the real **Font Awesome 6 Sharp Light** glyphs via the FA Pro kit (`kit.fontawesome.com/4b21e79d5d.js`, embedded in `<head>`): `calendar-clock` (Course schedule), `book-open-cover` (How you learn), `money-bill-wave` (Tuition), `file-circle-check` (Grading), `graduation-cap` (Programs), `book-open-reader` (Ideal for), plus `fa-solid fa-arrow-right` in CTAs. *Production note:* verify the kit's allowed-domains list includes the production hostname; a `ResizeObserver` re-anchors the glider when the kit's async SVG swap changes button widths.
